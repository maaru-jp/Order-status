...
**資料來源**：`MAARU 日本萌GO訂單進度資料表`（或同名結構試算表）

| 分頁 | 用途 |
|------|------|
| **工作表1** | 訂單編號｜商品內容｜圖片網址｜商品狀態｜出貨狀態 |
| **歷程** | 配送歷程（訂單編號｜狀態｜備註｜更新時間） |
| **狀態** | 選用狀態對照 |

顧客輸入 **五碼**（如 `00083`）→ API `order_status` → 優先讀 **工作表1** + **歷程**。

> **重要**：`Code.gs` 必須部署在**這份試算表**綁定的 Apps Script（擴充功能），並重新部署網頁應用程式。
